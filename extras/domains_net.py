...
genes_df = pd.read_csv(sys.argv[2], index_col=0)
...
def species_graph(sp_chrom):
    ...
    if not os.path.exists("{}/{}_{}.graphml".format(out_dir, sp, chrom)):
        ...
        dom_G = nx.Graph()
        sp_genes = genes_df.loc[(genes_df["species"] == sp) &
                                (genes_df["chromosome"] == chrom)].index
        for domain in domain_dict:
            genes_with_dom = [x for x in domain_dict[domain] if x in sp_genes]
